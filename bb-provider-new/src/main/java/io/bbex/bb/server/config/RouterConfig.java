...
@Configuration
@Slf4j
@MapperScan(basePackages = "io.bbex.bb.common.mapper.router", sqlSessionFactoryRef = "routerSessionFactory")
public class RouterConfig {

    //Below is the general config
    @Getter
    @Value("${has.zkAddress}")
    private String zkAddress;

    @Getter
    @Value("${has.useZkClient}")
    private Boolean useZkClient;

    @Getter
    @Value("${bb.location}")
    private String localtion;

    //Below is the data source config
    ...
    public RouterConfig(MybatisProperties myBatisProperties) {
    ...
    public DataSource routerDataSource(PrometheusHistogramMetricsTrackerFactory prometheusHistogramMetricsTrackerFactory) {
        BBMysqlDataSource dataSource = DataSourceBuilder.create()
                .url(url).username(username).password(password).driverClassName(driverClassName)
                .type(BBMysqlDataSource.class).build();
    ...
    @Bean
    @Primary
    public SqlSessionFactory routerSessionFactory(@Qualifier("routerDataSource") DataSource routerDataSource) throws Exception {
    ...
    @Bean
    public PlatformTransactionManager routerTransactionManager(@Qualifier("routerDataSource") DataSource routerDataSource) {
